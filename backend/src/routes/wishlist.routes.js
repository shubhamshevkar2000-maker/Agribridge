...
router.get('/', auth_middleware_1.protect, async (req, res) => {
    try {
        let wishlist = await Wishlist_1.Wishlist.findOne({ buyerId: req.user.id }).populate({
            path: 'cropIds',
            populate: {
                path: 'farmerId',
        ...
        if (!wishlist) {
            wishlist = await Wishlist_1.Wishlist.create({ buyerId: req.user.id, cropIds: [] });
        }
...
router.post('/toggle', auth_middleware_1.protect, async (req, res) => {
    try {
        ...
        let wishlist = await Wishlist_1.Wishlist.findOne({ buyerId: req.user.id });
        if (!wishlist) {
            wishlist = await Wishlist_1.Wishlist.create({ buyerId: req.user.id, cropIds: [] });
        }
...
router.post('/add', auth_middleware_1.protect, async (req, res) => {
    try {
        const { cropId } = req.body;
        if (!cropId) {
            return res.status(400).json({ success: false, message: 'cropId is required' });
...
router.post('/remove', auth_middleware_1.protect, async (req, res) => {
    try {
        const { cropId } = req.body;
        if (!cropId) {
            return res.status(400).json({ success: false, message: 'cropId is required' });
