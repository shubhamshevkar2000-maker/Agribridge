...
router.post('/:id/accept', auth_middleware_1.protect, async (req, res) => {
    try {
        const order = await Order_1.Order.findById(req.params.id);
        if (!order)
            return res.status(404).json({ success: false, message: 'Order not found' });
...
router.post('/:id/reject', auth_middleware_1.protect, async (req, res) => {
    try {
        const order = await Order_1.Order.findById(req.params.id);
        if (!order)
            return res.status(404).json({ success: false, message: 'Order not found' });
...
router.post('/:id/ready', auth_middleware_1.protect, async (req, res) => {
    try {
        const order = await Order_1.Order.findById(req.params.id);
        if (!order)
            return res.status(404).json({ success: false, message: 'Order not found' });
        ...
        const logisticsPartners = await User_1.User.find({ role: 'logistics' });
        if (logisticsPartners.length > 0) {
            delivery.logisticsPartnerId = logisticsPartners[0]._id;
        }
        else {
            const anyUser = await User_1.User.findOne();
            if (anyUser)
                delivery.logisticsPartnerId = anyUser._id;
        }
        const drivers = await User_1.User.find({ role: 'logistics' });
        if (drivers.length > 0) {
...
router.post('/:id/dispatch', auth_middleware_1.protect, async (req, res) => {
    try {
        const order = await Order_1.Order.findById(req.params.id);
        if (!order)
            return res.status(404).json({ success: false, message: 'Order not found' });
...
router.post('/:id/out-for-delivery', auth_middleware_1.protect, async (req, res) => {
    try {
        const order = await Order_1.Order.findById(req.params.id);
        if (!order)
            return res.status(404).json({ success: false, message: 'Order not found' });
