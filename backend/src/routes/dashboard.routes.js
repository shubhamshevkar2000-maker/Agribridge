...
router.get('/farmer', auth_middleware_1.protect, async (req, res) => {
    try {
        ...
        const orders = await Order_1.Order.find({ farmerId: userId, paymentStatus: 'completed' }).populate('cropId');
        ...
        const allCrops = await Crop_1.Crop.find({ farmerId: userId });
        ...
        const activeAuctions = await Auction_1.Auction.find({ farmerId: userId, status: 'live' }).populate('cropId').sort({ endTime: 1 });
        ...
        const notifications = await Notification_1.Notification.find({ userId: userId }).sort({ createdAt: -1 }).limit(10);
        const unreadNotifications = notifications.filter(n => !n.isRead).length;
        // 7. Fetch Recent Activity
        const recentActivity = await Transaction_1.Transaction.find({ payeeId: userId, status: 'success' })
            .sort({ timestamp: -1 })
            .limit(5)
...
router.get('/buyer', auth_middleware_1.protect, async (req, res) => {
    try {
        const userId = req.user.id;
        // 1. Total spent
        const completedOrders = await Order_1.Order.find({ buyerId: userId, paymentStatus: 'completed' });
        ...
        // 4. Recent Purchases
        const recentPurchases = await Order_1.Order.find({ buyerId: userId })
            .sort({ createdAt: -1 })
            .limit(5)
            .populate('cropId')
