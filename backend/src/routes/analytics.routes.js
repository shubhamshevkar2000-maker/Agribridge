...
router.get('/farmer', auth_middleware_1.protect, async (req, res) => {
    try {
        ...
        const completedOrders = await Order_1.Order.find({ farmerId: userId, paymentStatus: 'completed' }).populate('cropId');
        ...
        const farmerCrops = await Crop_1.Crop.find({ farmerId: userId, status: { $in: ['draft', 'listed'] } });
        ...
        const farmerAuctions = await Auction_1.Auction.find({ farmerId: userId });
