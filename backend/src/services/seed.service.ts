...
export const seedDemoData = async () => {
  ...
  let logistics = await User.findOne({ email: logisticsEmail });
  if (!logistics) {
    logistics = await User.create({
  ...
  let bank = await User.findOne({ email: bankEmail });
  if (!bank) {
    bank = await User.create({
  ...
  // 2. Create Crops
  let insertedCrops = await Crop.find({ isDemoAccount: true });
  if (insertedCrops.length === 0) {
    ...
    const cropsToInsert = cropMetadata.map(meta => ({
      ...
      status: 'listed' as 'draft' | 'listed' | 'in_auction' | 'sold' | 'expired',
      location: farmer!.location,
      isDemoAccount: true
    }));

    insertedCrops = await Crop.insertMany(cropsToInsert);
  ...
  const auctionCount = await Auction.countDocuments({ isDemoAccount: true });
  if (auctionCount === 0 && insertedCrops.length >= 3) {
    ...
    try {
      ...
      await redisClient.set(redisKey, '4000');
  ...
  const orderCount = await Order.countDocuments({ isDemoAccount: true });
  ...
  if (orderCount === 0 && insertedCrops.length > 0) {
    ...
    insertedOrders = await Order.find({ isDemoAccount: true });
    console.log('Demo orders already exist.');
  }

  // 5. Create Deliveries for some completed orders
  const deliveryCount = await Delivery.countDocuments({ isDemoAccount: true });
  ...
  const transactionCount = await Transaction.countDocuments({ isDemoAccount: true });
  ...
  // 7. Create Loans
  const loanCount = await Loan.countDocuments({ isDemoAccount: true });
  if (loanCount === 0) {
    await Loan.create({
      farmerId: farmer._id,
      bankId: bank._id,
  ...
  const wishlistCount = await Wishlist.countDocuments({ buyerId: buyer._id });
  ...
  const notificationCount = await Notification.countDocuments({ isDemoAccount: true });
...
export const clearDemoData = async () => {
  ...
  await Crop.deleteMany({ isDemoAccount: true });
  await Auction.deleteMany({ isDemoAccount: true });
  await Order.deleteMany({ isDemoAccount: true });
  await Delivery.deleteMany({ isDemoAccount: true });
  await Notification.deleteMany({ isDemoAccount: true });
  await Transaction.deleteMany({ isDemoAccount: true });
  await Loan.deleteMany({ isDemoAccount: true });
  await Wishlist.deleteMany({ isDemoAccount: true });
  console.log('Demo data cleared.');
};

export const seedDashboardData = async (shouldDisconnect = false) => {
  try {
    await seedDemoData();
  } catch (err) {
