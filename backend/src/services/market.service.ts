...
const extractCropAndLocation = (prompt: string, fallbackLocation?: string) => {
...
export const fetchMarketPrices = async (prompt: string, defaultLocation?: string): Promise<MarketData | null> => {
  const { crop, location } = extractCropAndLocation(prompt, defaultLocation);
  
  if (!crop) return null; // Unrecognized crop
