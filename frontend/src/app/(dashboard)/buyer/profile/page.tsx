...
export default function ProfilePage() {
  ...
  const calculateCompletion = () => {
    ...
    // 3. Buyer Preferences Details (20%)
    if (formData.buyerPreferences) score += 20;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload: any = {
        ...
        buyerPreferences: formData.buyerPreferences || undefined,
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex-1 max-w-xs space-y-2">
              ...
              {completion < 100 && (
                <p className="text-xs text-muted-foreground italic">Complete optional details to unlock bid refunds and payouts.</p>
              )}
            </div>
          </div>
        </CardContent>
          ...
          <Card className="glass-card border-border/50 shadow-md min-h-[350px]">
            <CardHeader>
              ...
              <CardDescription>
                ...
                {activeTab === 'preferences' && 'Set crop preferences, categories or quantities you want to purchase.'}
                {activeTab === 'bank' && 'Provide accounts for receiving bid refunds or processing transactions.'}
                {activeTab === 'notifications' && 'Select how you want to be kept up-to-date with purchase alerts.'}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <AnimatePresence mode="wait">
                ...
                {activeTab === 'preferences' && (
                  ...
                  >
                    <div className="space-y-1">
                      <label className="text-xs text-muted-foreground flex items-center gap-1"><FileText className="w-3.5 h-3.5" /> Buyer Preferences</label>
                      <textarea 
                        disabled={!isEditing} 
                        value={formData.buyerPreferences} 
                        onChange={e => setFormData({ ...formData, buyerPreferences: e.target.value })} 
                        className="w-full min-h-[120px] rounded-lg border border-border/50 bg-secondary/10 p-3 text-sm focus:ring-1 focus:ring-primary outline-none disabled:opacity-50"
                        placeholder="e.g. Interested in buying organic Wheat and Basmati Rice in bulk quantities (&gt;500 kg)."
                      />
                    </div>
                  </motion.div>
                )}
                ...
                {activeTab === 'notifications' && (
                  ...
                  >
                    <div className="space-y-3">
                      {[
                        { key: 'email', label: 'Receive Email updates for orders and bids' },
                        { key: 'sms', label: 'Receive SMS alerts for shipment arrivals' },
